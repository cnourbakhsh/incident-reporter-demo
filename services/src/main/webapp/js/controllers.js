(function () {
	...
	function mainController($log, $http, $interval) {
		...
		vm.supervisorUILabel = 'supervisor-ui';
		...
		function loadVars() {
			...
			$http({
				...
				url: location.protocol + '//' + location.host + '/' + vm.supervisorUILabel + '/bpm/kie-server/services/rest/server/queries/processes/instances/' + vm.processId + '?withVars=true',
				headers: {
				...
				if (data["process-instance-variables"]["incidentPhotoIds"]) {
					...
					for (var i = 0; i < photoIdArr.length; i++) {
						...
						vm.photos.push(location.protocol + '//' + location.host + '/' + vm.supervisorUILabel + '/photos/' + vm.processId + '/' + photoName);
					}
				}
		...
		function loadImage() {
			var targetUrl = location.protocol + '//' + location.host + '/' + vm.supervisorUILabel + '/bpm/kie-server/services/rest/server/containers/' + vm.containerId + "/images/processes/instances/" + vm.processId;
		...
		function submit() {
			var baseUrl = location.protocol + '//' + location.host + '/' + vm.supervisorUILabel + '/bpm/kie-server/services/rest/server/';
			$log.info('Approving');
			$http({
				method: 'GET',
				withCredentials: true,
